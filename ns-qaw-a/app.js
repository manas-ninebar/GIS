...
import { parseAsk, starters } from './chat.js'
...
function paint() {
  ...
  state.geo = buildGeo(sites, cells, { bandPin, selectedId: state.selected })
  if (state.map) {
...
function renderCard() {
  ...
  el.hidden = false
  el.innerHTML = `
    <button type="button" class="x" id="card-x" aria-label="Close">×</button>
    ...
    <table>
      <tr><th>Cell</th><th>PCI</th><th>Az</th><th>ECGI</th></tr>
      ${cells.map((c) => `<tr class="row-hit" data-cell="${c.cell_id}"><td>${v(c.cell_name)}</td><td>${v(c.pci)}</td><td>${v(c.azimuth)}°</td><td>${v(c.ecgi)}</td></tr>`).join('')}
    </table>
    ${alarms || ''}
    ${site.note ? `<div class="prov">${site.note}</div>` : ''}
    <div class="prov">${Number(v(site.lat)).toFixed(5)} N · ${Number(v(site.lng)).toFixed(5)} E ← cell-plan</div>
    <div class="prov">${state.inv.clock?.t || '—'} ← ${state.inv.clock?.source || 'clock'} · ${cells.length} cells · WGS84</div>
  `
...
function applyIntent(intent) {
  if (intent.type === 'recipe') {
    state.recipe = { ...defaultRecipe(), ...intent.recipe }
  ...
  } else if (intent.type === 'qa' && intent.site) {
    state.selected = intent.site.site_id
    paint()
  }
  if (intent.narrate) logMsg(intent.narrate)
}
...
async function boot() {
  ...
  const finish = () => {
    ...
    if ($('clock-label')) {
      $('clock-label').textContent = clk?.t
        ? `Snapshot · ${clk.t} ← ${clk.source}`
        : 'Snapshot · cell-plan'
    }
    logMsg(`<b>Instrument, not catalogue.</b> MapLibre · deck.gl GPU · one clock. ${c.sites} rooftops · ${c.gh.toLocaleString()} GH samples · ${c.dt.toLocaleString()} DT samples. 2D until 3D earns its place.`)
  }
  state.map = createMap($('map'), { view: state.recipe.view, onLoad: finish })
  window.__map = state.map
  setTimeout(() => { if (!booted) finish() }, 1200)
  state.map.on('mousemove', (e) => {
    $('hud').textContent = `${e.lngLat.lat.toFixed(5)} N   ${e.lngLat.lng.toFixed(5)} E`
  })
  ...
  $('starters').querySelectorAll('button').forEach((b) => {
    b.onclick = () => { $('ask').value = b.textContent; $('composer').requestSubmit() }
  })
  $('composer').addEventListener('submit', (e) => {
    ...
    if (!q) return
    logMsg(q, 'user')
    applyIntent(parseAsk(q, state.inv, state.selected))
    $('ask').value = ''
  })
  $('btn-rail').onclick = () => toggle('rail')
  $('btn-copilot').onclick = () => toggle('copilot')
  $('rail-x').onclick = () => { $('rail').hidden = true }
  $('copilot-x').onclick = () => { $('copilot').hidden = true }
